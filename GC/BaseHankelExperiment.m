function BaseHankelExperiment(expnum, nvars, mats, randpfn, randvfn, ...
...
[U1, ~, V1] = preprocfn(Yhist(:, :, 1, 1));
[stackedVars, ~, ~] = size(U1);
[stackedNobs, rank, ~] = size(V1);

Xhist = zeros(rank, stackedNobs, ntrials, reps);
Uhist = zeros(stackedVars, rank, ntrials, reps);
Shist = zeros(rank, rank, ntrials, reps);
Vhist = zeros(stackedNobs, rank, ntrials, reps);

% est holds network inference method's estimate of the networks
est = zeros(rank, rank, reps, numSplits);
...
for j = 1 : numMats
    ...
    for r = 1 : reps
        ...
        Y = GenerateNNCoupledData(nvars, tSpan, ntrials, randpfn, randvfn, randmfn, randkfn, randcfn, 'fixed');
        %X = preprocfn(Y);
        [U, S, V] = preprocfn(Y);
        plot(V(:, :, 1))
        X = permute(V, [2, 1, 3]);
