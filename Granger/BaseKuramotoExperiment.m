function BaseKuramotoExperiment(expnum, mats, Kvals, pfn, wfn, cfn, ...
    kickTime, pert, preprocfn, deltat, endtime, ntrials, reps, tsplits, ...
    freq, networkInferenceFn, numDiagnostics)
...
%       of (possibly random) initial positions p, one for each node
%       in the network
%
% wfn 
%       a function that takes a scalar n and returns an [n x 1] vector 
%       of (possibly random) initial frequencies w, one for each node in
%       the network 
%
% preprocfn
...
for j = 1:numMats
    ...
    for k = 1:nKvals
        ...
        for r = 1:reps
            ...
            Y = GenerateKuramotoData(truth, tSpan, ntrials, K, pfn, wfn, cfn, kickTime, pert);
            [theta, X] = preprocfn(Y);
            ...
            for s = 1:numSplits
                ...
                try
                    [est(:,:,r,s), diagnostics] = networkInferenceFn(theta(:,beg:tsplits(s),:));
                catch ME
                    % If exception is caught, report its error message and
                    % save an empty connectivity matrix.
                    getReport(ME)
                end
