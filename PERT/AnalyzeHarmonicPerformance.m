...
for numObs = 1:nvars
    ...
    for numPerts = 1:maxNumPerts
        ...
        if pertIsObs
            pertIdx = randsample(obsIdx, numPerts);
        else
        ...
        while trial <= numTrials
            ...
            dataLog{numPerts, trial, 3} = obsIdx;
            dataLog{numPerts, trial, 4} = pertIdx;
            dataLog{numPerts, trial, 5} = pertTimes;
            dataLog{numPerts, trial, 6} = pertLength;
...
for numObs = nvars:-1:1
    numObs
    for numPerts = maxNumPerts:-1:1
        numPerts
        for trial = 1:numTrials
            data = dataLog{numPerts, trial, 1};
            mat = dataLog{numPerts, trial, 2};
            obsIdx = dataLog{numPerts, trial, 3};
            pertIdx = dataLog{numPerts, trial, 4};
            pertTimes = dataLog{numPerts, trial, 5};
            pertLength = dataLog{numPerts, trial, 6};
